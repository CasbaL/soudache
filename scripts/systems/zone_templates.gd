## 区域模板数据
## 存储各层各类型区域的敌人配置、资源、NPC对话等
## 基于原房间模板数据重构为区域模板
class_name ZoneTemplates
extends RefCounted
...
## 获取指定层、指定区域类型的模板列表
static func get_templates(layer: int, zone_type) -> Array[Dictionary]:
	var ZT = load("res://scripts/systems/map_zone.gd")
	match layer:
		1:
			match zone_type:
				ZT.ZoneType.COMBAT:
					return get_layer1_combat_templates()
				ZT.ZoneType.RESOURCE:
...
## 从模板随机选择一个
static func pick_random_template(layer: int, zone_type) -> Dictionary:
	var templates = get_templates(layer, zone_type)
